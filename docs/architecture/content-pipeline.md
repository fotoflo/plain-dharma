...
**Pause markers:** Audio pauses are authored in the canonical MDX as **invisible comments**:
```mdx
{/* pause */}       ← 1.0 second break
{/* pause:2.5 */}   ← custom duration
```
These are converted to ElevenLabs `<break time="Ns" />` tags at generation time (for narration),
and stripped from PDF/print (`scripts/lib/book-source.ts`) and mobile (`apps/mobile/src/content/markdown.ts`)
by `stripPauseMarkers()`. Web rendering ignores them (comments are invisible).
