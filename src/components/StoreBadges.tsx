 ...
 * guidelines. Renders nothing until `APP_LINKS.published` flips true on store
 * approval — so it can sit on any page now and light up at launch.
 *
 * Plain <img> (not next/image): no point optimizing a vector badge, and it
 * sidesteps the images.localPatterns config.
 */
export function StoreBadges({ className = "" }: { className?: string }) {
  if (!APP_LINKS.published) return null;
  return (
    <div
