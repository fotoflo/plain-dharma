// Platform-agnostic content registry shared by web and mobile. No bundler- or
// runtime-specific code here (no MDX loaders, no fs) — web keeps its `.mdx`
// LOADERS and mobile inline-imports the same `.mdx` files as raw strings.
...
// Both supported locales provide every sutta. Derived from SUTTA_DISPLAY (the
// per-locale content map) rather than a bundler-specific loader map, so it
// stays platform-agnostic.
export function getAvailableLocales(slug: SuttaSlug): Locale[] {
  return SUPPORTED_LOCALES.filter((loc) => Boolean(SUTTA_DISPLAY[loc]?.[slug]));
}
